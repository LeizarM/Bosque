/* ============================================================================
   TPEX · Corregir COMPROBANTE de una transacción (N° / fecha valor / voucher)
   Fecha: 2026-06-23 (F) — EJECUTAR CON USUARIO DE ESCRITURA
   Requiere: migración E de 2026-06-23 (esta reproduce el SP de la E + la ACCION 'C').

   Problema: una transacción ya CONFIRMADA queda bloqueada para todo cambio
   (ACCION 'U' → error 52). Pero a veces hay que corregir SOLO el comprobante:
   el N° de transacción bancaria, la fecha valor o el voucher subido.

   Solución: nueva ACCION 'C' que actualiza ÚNICAMENTE numeroTransaccion,
   fechaValor y rutaVoucher. NO reabre el pago ni toca montos/cotización/estado.
   Permitida incluso en CONFIRMADO (solo se bloquea en CANCELADO). Errores 70-72.

   Solo se re-altera p_abm_tpex_Transacciones; cambio respecto a la E:
     - se agrega el bloque de la ACCION 'C' al final (antes del END del procedure).
   ============================================================================ */
...
BEGIN
    ...
    -- Guard de cuadre (al confirmar)
    DECLARE @gCntAsientos INT;
    DECLARE @gTotalDebBs DECIMAL(18, 4);
    DECLARE @gTotalCreBs DECIMAL(18, 4);
        ...
        BEGIN
            ...
            IF ISNULL(@estado, '') != '' AND @estado != @estadoActual
                BEGIN
                    ...
                    -- ── GUARD DE CUADRE: el pago va DESPUÉS de los asientos ──────────
                    -- Al confirmar, exigir que existan asientos y que cuadren (Debe=Haber).
                    IF @estado = 'CONFIRMADO'
                    BEGIN
                        SELECT @gCntAsientos = COUNT(*),
                               @gTotalDebBs  = ISNULL(SUM(debitoBs), 0),
                               @gTotalCreBs  = ISNULL(SUM(creditoBs), 0)
                        FROM tpex_Asientos
                        WHERE idTransaccion = @idTransaccion AND eliminado = 0;

                        IF @gCntAsientos = 0
                        BEGIN
                            SET @error = 54;
                            SET @errormsg = 'No se puede confirmar: la transacción no tiene asientos contables (Debe/Haber). Cárguelos en Cobranzas — Asientos.';
                            RETURN;
                        END

                        IF ABS(@gTotalDebBs - @gTotalCreBs) >= 0.01
                        BEGIN
                            SET @error = 55;
                            SET @errormsg = 'No se puede confirmar: los asientos no estan CUADRADOS (Debito=' +
                                            CAST(@gTotalDebBs AS VARCHAR(30)) + ' / Credito=' + CAST(@gTotalCreBs AS VARCHAR(30)) +
                                            '). Ajuste el Debe/Haber hasta que la diferencia sea 0.';
                            RETURN;
                        END
                    END
                END
    ...
    --   Permitida incluso en CONFIRMADO. NO reabre el pago ni toca montos,
    --   cotización ni estado. Solo actualiza numeroTransaccion / fechaValor /
    --   rutaVoucher. Se bloquea únicamente si la transacción está CANCELADA.
    -- ================================================================
    IF @ACCION = 'C'
        BEGIN
            IF ISNULL(@idTransaccion, 0) = 0
                BEGIN SET @error = 70; SET @errormsg = 'idTransaccion es obligatorio para corregir el comprobante.'; RETURN; END

            SELECT @estadoActual = estado FROM tpex_Transacciones WHERE idTransaccion = @idTransaccion AND eliminado = 0;
            IF @estadoActual IS NULL
                BEGIN SET @error = 71; SET @errormsg = 'No existe la transacción ' + CAST(@idTransaccion AS VARCHAR) + '.'; RETURN; END
            IF @estadoActual = 'CANCELADO'
                BEGIN SET @error = 72; SET @errormsg = 'No se puede corregir el comprobante de una transacción CANCELADA.'; RETURN; END

            BEGIN TRY
                BEGIN TRANSACTION;
                UPDATE tpex_Transacciones
                SET numeroTransaccion = ISNULL(@numeroTransaccion, numeroTransaccion),
                    fechaValor        = ISNULL(@fechaValor,        fechaValor),
                    rutaVoucher       = ISNULL(@rutaVoucher,       rutaVoucher),
                    audUsuario        = @audUsuario,
                    audFecha          = GETDATE()
                WHERE idTransaccion = @idTransaccion;
                ...
                INSERT INTO dbo.tpex_LogEstados (idSolicitud, idCotizacion, idTransaccion, estadoAnterior, estadoNuevo, observaciones, audUsuario, audFecha)
                VALUES (NULL, NULL, @idTransaccion, @estadoActual, @estadoActual, 'Comprobante corregido (N°/fecha valor/voucher).', @audUsuario, GETDATE());

                SET @idGenerado = @idTransaccion;
                COMMIT TRANSACTION;
                SET @error    = 0;
                SET @errormsg = 'Comprobante actualizado (N° / fecha valor / voucher).';
            END TRY
            BEGIN CATCH
                IF @@TRANCOUNT > 0 ROLLBACK TRANSACTION;
                SET @error    = 99;
                SET @errormsg = 'Error al corregir comprobante: ' + ERROR_MESSAGE() + ' | Línea: ' + CAST(ERROR_LINE() AS VARCHAR);
            END CATCH;
            RETURN;
        END
END
