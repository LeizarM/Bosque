-- ============================================================================
--  FIX: p_abm_tpex_Transacciones — conversión de moneda invertida
--  EJECUTAR EN: BOSQUE-2_0 (SQL Server 2008)
--  Fecha: 2026-06-13
--
--  PROBLEMA (detectado en QA del 12/06):
--    La UI mostraba montoConvertido = 34.850 (5.000 x 6,97) pero la BD guardaba
--    717,36 (5.000 / 6,97). El SP dividía cuando debe multiplicar.
--
--  EVIDENCIA (planillas Excel + coherencia interna del propio SP):
--    - Excel FORWARD CARVAJAL: MONTO USD 60.000 x TC 6,97 = 418.200 Bs.  → multiplica.
--    - Cotización (p_abm_tpex_Cotizaciones / UI): totalBolivianos = montoCompra x TC.
--    - El propio SP, en equivalenteUsdRef = totalFinal / tipoCambioReferencia y
--      diferenciaDeMas = equivalenteUsdRef - montoOrigen, SOLO cuadra si montoOrigen
--      está en divisa (USD) y montoConvertido en Bs → es decir, multiplicación.
--      La línea de división contradecía al resto del propio procedimiento.
--
--  MODELO CORRECTO (confirmado):
--    montoOrigen      = monto en la DIVISA que se compra/paga (USD, EUR, BRL, USDT)
--    monedaOrigen     = la divisa  (lo que la UI precarga desde la cotización)
--    montoConvertido  = monto en bolivianos = montoOrigen * tipoCambioAplicado
--    monedaDestino    = BOB
--
--  CAMBIOS vs. la versión vigente (resto del SP intacto):
--    1. Validación de monedas para TC_DIRECTO/TC_NEGOCIADO/FORWARD:
--       antes exigía origen = BOB; ahora exige origen = divisa y destino = BOB
--       (se alinea con la precarga de la UI y evita el falso error 39).
--    2. Cálculo: montoConvertido = montoOrigen * TC   (antes: / TC).
--
--  Tipos especiales (no entran en la validación de monedas y la multiplicación
--  les aplica igual de bien, verificado contra las planillas):
--    FONDEO_MERCURY  (Bs<-USD): montoOrigen=USD x TC = Bs.
--    COMPRA_USDT     (Bs<-USDT): montoOrigen=USDT x TC = Bs.
--    TRASPASO_MERCURY (USD<->USD): con TC=1 montoConvertido = montoOrigen; con TC
--                     el equivalente Bs, consistente con cómo se registran.
--    DEVOLUCION / EXPORTADORA: mismo criterio (divisa x TC).
-- ============================================================================
USE [BOSQUE-2_0];
GO
...
-- ============================================================================
-- LIMPIEZA: la transacción de prueba #1 (solicitud 2) se guardó con el cálculo
-- viejo (montoConvertido = 717,36). Recálculo manual a valor correcto para no
-- dejar datos sucios en DEV. Si prefieres borrarla, usa el DELETE comentado.
-- ============================================================================
UPDATE tpex_Transacciones
SET montoConvertido = ROUND(montoOrigen * tipoCambioAplicado, 2),
    totalFinal      = ROUND(montoOrigen * tipoCambioAplicado + ISNULL(totalCargos, 0), 2)
WHERE idSolicitud = 2
  AND estado = 'PENDIENTE'
  AND ABS(montoConvertido - ROUND(montoOrigen * tipoCambioAplicado, 2)) > 0.01;
GO

-- Alternativa (borrar la transacción de prueba en vez de corregirla):
-- DELETE FROM tpex_LogEstados   WHERE idTransaccion IN (SELECT idTransaccion FROM tpex_Transacciones WHERE idSolicitud = 2);
-- DELETE FROM tpex_Transacciones WHERE idSolicitud = 2;
