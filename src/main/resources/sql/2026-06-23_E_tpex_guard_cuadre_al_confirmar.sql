/* ============================================================================
   TPEX · Guard de CUADRE al confirmar la transacción (backend, a prueba de balas)
   Fecha: 2026-06-23 (E) — EJECUTAR CON USUARIO DE ESCRITURA
   Requiere: migración D de 2026-06-23 (esta reproduce el SP de la D + el guard).

   Regla de orden: transacción → asientos → pago.
   Al pasar la transacción a CONFIRMADO (ACCION 'U' con @estado='CONFIRMADO'),
   el SP ahora exige que:
     1) la transacción tenga al menos 1 asiento contable (Debe/Haber), y
     2) los asientos estén CUADRADOS (SUM(debitoBs) = SUM(creditoBs)).
   Si no, devuelve error y NO confirma. Cubre las dos vías (confirmar-pago y
   cambiar-estado), incluso llamadas directas a la API.

   Solo se re-altera p_abm_tpex_Transacciones; cambios respecto a la D:
     - 3 variables nuevas en el DECLARE (@gCntAsientos, @gTotalDebBs, @gTotalCreBs).
     - bloque de validación de cuadre en ACCION 'U' al transicionar a CONFIRMADO
       (errores 54 y 55).
   ============================================================================ */
