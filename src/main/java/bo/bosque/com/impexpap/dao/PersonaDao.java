...
public class PersonaDao implements IPersona {
    ...
    /**
     * Procedimiento para obtener los datos personales de una persona mediante el CI
     * @param codPersona
     * @return
     */
    public Persona obtenerDatosXCarnet(String ciNumero) {
        Persona per;
        try {
            per =  this.jdbcTemplate.queryForObject("execute p_list_Persona @ciNumero=?, @ACCION=?",
                    new Object[] { ciNumero, "L" },
                    new int[] { Types.INTEGER, Types.VARCHAR },
                    (rs, rowNum) -> {
                        Persona temp = new Persona();
                        temp.setCodPersona(rs.getInt(1));
